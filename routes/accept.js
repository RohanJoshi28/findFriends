...
router.get("/", function(req,res,next){
    ...
    var req_queries = Object.keys(req.query);
    ...
    for (var i=0; i<req_queries.length; i++){
        if (req_queries[i]=="person"){
            continue; 
        } else if (req.query[req_queries[i]]=="on"){
    ...
    for (var i=0; i<friend_reqs_on.length; i++){
        ...
        user_data[friend_reqs_on[i]]["Sent"] = user_data[friend_reqs_on[i]]["Sent"].filter(function(value, index, arr){
            return value!=person.split(" ")[0];
        });

        //remove the friend from the user's "recieved" array
        user_data[person.split(" ")[0]]["Recieved"] = user_data[person.split(" ")[0]]["Recieved"].filter(function(value, index, arr){
            return !(friend_reqs_on.includes(value));
        });

        //add them as each other's friends
        user_data[person.split(" ")[0]]["Friends"].push(friend_reqs_on[i]);
        user_data[friend_reqs_on[i]]["Friends"].push(person.split(" ")[0])
    }
    ...
    res.render("accepted.hbs", {name: person.split(" ")[0], password: person.split(" ")[1], people: friend_reqs_on});
